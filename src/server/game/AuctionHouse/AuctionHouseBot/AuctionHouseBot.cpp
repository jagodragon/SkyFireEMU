...
AuctionHouseBot ACE_Singleton_1( AuctionHouseBot );
AuctionBotConfig ACE_Singleton_1( AuctionBotConfig );

//== AuctionBotConfig functions ============================

AuctionBotConfig::AuctionBotConfig() : m_configFileName((std::string)"ahbot.conf")
{
}

bool AuctionBotConfig::Initialize()
{
    if (!ConfigMgr::Load(m_configFileName.c_str()))
    {
...
void AuctionBotConfig::GetConfigFromFile()
{
    ...
    setConfig(CONFIG_BOOL_AHBOT_OVERRIDE_SIDE                , "AuctionHouseBot.OverrideSide"                , false);
...
{ 
    char const* tempare[MAX_ITEM_CLASS] = { "CONSUMABLE", "CONTAINER", "WEAPON", "GEM", "ARMOR", "REAGENT", "PROJECTILE", "GOODS", "GENERIC", "RECIPE", "MONEY", "QUIVER", "QUEST", "KEY", "PERMANENT", "MISC", "GLYPH" };
...
bool AuctionBotBuyer::Initialize()
{
    ...
    if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
        sLog->outString("AHBot buyer interval between 2 check = %u", m_CheckInterval);
    // sLog->outString, !sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER));
    return true;
}
...
uint32 AuctionBotBuyer::GetBuyableEntry(AHB_Buyer_Config& config)
{
    ...
    if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
    {
        sLog->outString("AHBot: %u items added to buyable vector for ah type: %u",count, config.GetHouseType());
        sLog->outString("AHBot: SameItemInfo size = %u", config.SameItemInfo.size());
    }
    return count;
}
...
void AuctionBotBuyer::PrepareListOfEntry(AHB_Buyer_Config& config)
{
    ...
    if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
    {
        sLog->outString("AHBot: CheckedEntry size = %u",config.CheckedEntry.size());
    }
}
...
bool AuctionBotBuyer::IsBuyableEntry(uint32 buyoutPrice, double InGame_BuyPrice, double MaxBuyablePrice, uint32 MinBuyPrice, uint32 MaxChance, uint32 ChanceRatio)
{
    ...
    {
        if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
            sLog->outString("AHBot: WIN BUY! Chance = %u, num = %u.",Chance, RandNum);
        return true;
    }
    else
    {
        if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
            sLog->outString("AHBot:LOOSE BUY! Chance = %u, num = %u.",Chance, RandNum);
        return false;
    }
}
...
bool AuctionBotBuyer::IsBidableEntry(uint32 bidPrice, double InGame_BuyPrice, double MaxBidablePrice, uint32 MinBidPrice, uint32 MaxChance, uint32 ChanceRatio)
{
    ...
    {
        if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
            sLog->outString("AHBot: WIN BID! Chance = %u, num = %u.",Chance, RandNum);
        return true;
    } else
    {
        if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
            sLog->outString("AHBot: LOOSE BID! Chance = %u, num = %u.",Chance, RandNum);
        return false;
    }
}
...
{
    if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
        sLog->outString("AHBot: Bid placed to entry %u, %.2fg", auction->Id, float(bidPrice) / 10000.0f);
}

void AuctionBotBuyer::BuyEntry(AuctionEntry* auction)
{
    if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
        sLog->outString("AHBot: Entry %u buyed at %.2fg", auction->Id, float(auction->buyout) / 10000.0f);
...
void AuctionBotBuyer::addNewAuctionBuyerBotBid(AHB_Buyer_Config& config)
{
    ...
    if (config.CheckedEntry.size() > sAuctionBotConfig.GetItemPerCycleBoost())
    {
        ...
        sLog->outString("AHBot: Boost value used for Buyer! (if this happens often adjust both ItemsPerCycle in ahbot.conf)");
    }
    else
    ...
    for (CheckEntryMap::iterator itr =config.CheckedEntry.begin(); itr != config.CheckedEntry.end();)
    {
        ...
        {
            if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
                sLog->outString("AHBot: Entry %u on ah %u doesn't exists, perhaps bought already?",
        ...
        {
            if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
                sLog->outString("AHBot: In time interval wait for entry %u!", auction->Id);
            ++itr;
           continue;
        }
        ...
        if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
        {
            sLog->outString("AHBot: Auction added with data:");
            sLog->outString("AHBot: MaxPrice of Entry %u is %.1fg.", itr->second.AuctionId, MaxBuyablePrice / 10000);
            sLog->outString("AHBot: GamePrice buy=%.1fg, bid=%.1fg.",InGame_BuyPrice/10000, InGame_BidPrice / 10000);
            sLog->outString("AHBot: Minimal price see in AH Buy=%ug, Bid=%ug.",
                sameitem_itr->second.MinBuyPrice / 10000,sameitem_itr->second.MinBidPrice / 10000);
            sLog->outString("AHBot: Actual Entry price,  Buy=%ug, Bid=%ug.", buyoutPrice / 10000, bidPrice/ 10000);
        }
...
bool AuctionBotBuyer::Update(AuctionHouseType houseType)
{
    ...
    {
        if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
            sLog->outString("AHBot: %s buying ...", AuctionBotConfig::GetHouseTypeName(houseType));
...
bool AuctionBotSeller::Initialize()
{
    ...
    if (QueryResult result = WorldDatabase.Query("SELECT DISTINCT item FROM npc_vendor"))
    {
       ...
       if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
            sLog->outString("Numbers of lines to read [%u]",result->GetRowCount());
        do
        {
    ...
    {
    // Set a good error here     if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_BUYER))
    }
   ...
   if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
    ...
    if (QueryResult result2 = WorldDatabase.PQuery(
        ...
        "SELECT item FROM spell_loot_template"))
    {
        // BarGoLink bar(result->GetRowCount());
        if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
            sLog->outString("Numbers of lines to read[%u]",result2->GetRowCount());
        do
        {
    ...
    for (uint32 itemID = 0; itemID < sItemStorage.MaxEntry; ++itemID)
    {
        ...
        if(prototype->ItemId != itemID || itemID == 0)
        {
            ++itemsId;
            continue;
        }
       ...
       /* // Quality filters
        ...
        {
            case 0:
                if (sAuctionBotConfig.getConfig(CONFIG_UINT32_AHBOT_ITEM_GREY_AMOUNT) == 0)
                    continue;
                break;

            case 1:
                if (sAuctionBotConfig.getConfig(CONFIG_UINT32_AHBOT_ITEM_WHITE_AMOUNT) == 0)
                    continue;
                break;

            case 2:
                if (sAuctionBotConfig.getConfig(CONFIG_UINT32_AHBOT_ITEM_GREEN_AMOUNT) == 0)
                    continue;
                break;

            case 3:
                if (sAuctionBotConfig.getConfig(CONFIG_UINT32_AHBOT_ITEM_BLUE_AMOUNT) == 0)
                    continue;
                break;
                
            case 4:
                if (sAuctionBotConfig.getConfig(CONFIG_UINT32_AHBOT_ITEM_PURPLE_AMOUNT) == 0)
                    continue;
                break;
            
            case 5:
                if (sAuctionBotConfig.getConfig(CONFIG_UINT32_AHBOT_ITEM_ORANGE_AMOUNT) == 0)
                    continue;
                break;

            case 6:
                if (sAuctionBotConfig.getConfig(CONFIG_UINT32_AHBOT_ITEM_YELLOW_AMOUNT) == 0)
                    continue;
                break;
        ...
        */
        ...
        if (!sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_ITEMS_VENDOR))
        {
           ...
           /* if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
                sLog->outString("[ITEM : %u] is it a VendorItem %i", itemID, isVendorItem);
                */
            if (isVendorItem)
            {
                ++itemVendor;
   ...
   sLog->outString("AuctionHouseBot seller will use %u items to fill auction house (according your config choices)", itemsAdded);
    ...
    if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
    {
        ...
        for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
            sLog->outString("%-14s\t" SIZEFMTD "\t" SIZEFMTD "\t" SIZEFMTD "\t" SIZEFMTD "\t" SIZEFMTD "\t" SIZEFMTD "\t" SIZEFMTD,
        ...
        sLog->outString("Skipped -> Vendor items [%u],  Misc items [%u], Loot items [%u], itempPrototype [%u]", itemVendor, itemMisc, itemLoot, itemPrototype);
        sLog->outString("Skipped -> Item Name [%u], Item ID [%u]", itemsTest, itemsId);
        sLog->outString("Used items -> [%u]", itemsAdded);
    }
    sLog->outString();
...
void AuctionBotSeller::LoadSellerValues(AHB_Seller_Config& config)
{
   ...
   config.SetPriceRatioPerQuality(AUCTION_QUALITY_YELLOW,PriceRatio);
    ...
    sLog->outString("AHBot: minTime = %u", config.GetMinTime());
    sLog->outString("AHBot: maxTime = %u", config.GetMaxTime());

    sLog->outString("AHBot: For AH type %u",config.GetHouseType());
    sLog->outString("AHBot: GreyItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_GREY));
    sLog->outString("AHBot: WhiteItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_WHITE));
    sLog->outString("AHBot: GreenItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_GREEN));
    sLog->outString("AHBot: BlueItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_BLUE));
    sLog->outString("AHBot: PurpleItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_PURPLE));
    sLog->outString("AHBot: OrangeItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_ORANGE));
    sLog->outString("AHBot: YellowItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_YELLOW));
...
uint32 AuctionBotSeller::SetStat(AHB_Seller_Config& config)
{
    ...
    for (uint32 j=0; j<MAX_AUCTION_QUALITY; ++j)
    {
        for (uint32 i=0;i<MAX_ITEM_CLASS;++i)
        {
            ...
            {
                config.SetMissedItemsPerClass((AuctionQuality) j, (ItemClass) i, (uint32) ItemsInAH[j][i]);
                // sLog->outBasic("Missing %u from AH [%u]", config.GetMissedItemsPerClass((AuctionQuality) j, (ItemClass) i), ItemsInAH[j][i]);
    ...
    {
        sLog->outString("");
        sLog->outString("AHBot: Missed Item\tGrey\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
        for (uint32 i=0; i<MAX_ITEM_CLASS;++i)
        {
            sLog->outString("AHBot: %-14s\t%u[%u]\t%u[%u]\t%u[%u]\t%u[%u]\t%u[%u]\t%u[%u]\t%u[%u]",
            sAuctionBotConfig.GetItemClassName(ItemClass(i)),
            config.GetMissedItemsPerClass(AUCTION_QUALITY_GREY, (ItemClass) i), ItemsInAH[0][i],
            config.GetMissedItemsPerClass(AUCTION_QUALITY_WHITE, (ItemClass) i), ItemsInAH[1][i],
            config.GetMissedItemsPerClass(AUCTION_QUALITY_GREEN, (ItemClass) i), ItemsInAH[2][i],
            config.GetMissedItemsPerClass(AUCTION_QUALITY_BLUE, (ItemClass) i), ItemsInAH[3][i],
            config.GetMissedItemsPerClass(AUCTION_QUALITY_PURPLE, (ItemClass) i), ItemsInAH[4][i],
            config.GetMissedItemsPerClass(AUCTION_QUALITY_ORANGE, (ItemClass) i), ItemsInAH[5][i],
            config.GetMissedItemsPerClass(AUCTION_QUALITY_YELLOW, (ItemClass) i), ItemsInAH[6][i]);
  
        }
        sLog->outString("--------------[ Rest of total count [%u] ]-------------- ", count);
    }
...
void AuctionBotSeller::addNewAuctions(AHB_Seller_Config& config)
{
    ...
    else
    {
        ...
        // sLog->outString("AHBot: Normal value used to fill AH");
    }
   ...
   uint32 Numbers = 0;
    ...
    while (getRandomArray(config, randArray, ItemsAdded) && (items>0))
    {
        ...
        if (!m_ItemPool[randArray[pos].color][randArray[pos].itemclass].empty())
        {   
        ...
        {
            if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
            sLog->outString("AHBot: Item entry 0 auction creating attempt.");
            ++itemTestID;
            continue;
        }
        ...
        {
            if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
                sLog->outString("AHBot: Unknown item %u auction creating attempt.", itemID);
            continue;
        }
        ...
        {
            if(sAuctionBotConfig.getConfig(CONFIG_BOOL_AHBOT_DEBUG_SELLER))
                sLog->outString("AHBot: Item::CreateItem() returned NULL for item %u (stack: %u)", itemID, stackCount);
            continue;
        }
...
void AuctionHouseBot::Rebuild(bool all)
{
    for (uint32 i = 0; i < MAX_AUCTION_HOUSE_TYPE; ++i)
    {
        AuctionHouseObject::AuctionEntryMapBounds bounds = sAuctionMgr->GetAuctionsMap(AuctionHouseType(i))->GetAuctionsBounds();
        for (AuctionHouseObject::AuctionEntryMap::const_iterator itr = bounds.first; itr != bounds.second; ++itr)
            if (itr->second->owner == 0)                        // ahbot auction
