...
bool ChatHandler::HandleAHBotItemsAmountCommand(const char* args)
{
  ...
  /*  for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
    ...
    for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
        PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, GetSkyFireString(ahbotQualityIds[i]), sAuctionBotConfig.getConfigItemQualityAmount(AuctionQuality(i)));
...
template<int Q>
bool ChatHandler::HandleAHBotItemsAmountQualityCommand(const char* args)
{
    ...
    if((qVal = (uint32) strtoul(ahMapIdStr, NULL, 0)) == false)
        return false;

    sAuctionBot.SetItemsAmountForQuality(AuctionQuality(Q), qVal);
    PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, GetSkyFireString(ahbotQualityIds[Q]),
    sAuctionBotConfig.getConfigItemQualityAmount(AuctionQuality(Q)));
    return true;
}

template bool ChatHandler::HandleAHBotItemsAmountQualityCommand<AUCTION_QUALITY_GREY>(const char*);
template bool ChatHandler::HandleAHBotItemsAmountQualityCommand<AUCTION_QUALITY_WHITE>(const char*);
template bool ChatHandler::HandleAHBotItemsAmountQualityCommand<AUCTION_QUALITY_GREEN>(const char*);
template bool ChatHandler::HandleAHBotItemsAmountQualityCommand<AUCTION_QUALITY_BLUE>(const char*);
template bool ChatHandler::HandleAHBotItemsAmountQualityCommand<AUCTION_QUALITY_PURPLE>(const char*);
template bool ChatHandler::HandleAHBotItemsAmountQualityCommand<AUCTION_QUALITY_ORANGE>(const char*);
template bool ChatHandler::HandleAHBotItemsAmountQualityCommand<AUCTION_QUALITY_YELLOW>(const char*);

bool ChatHandler::HandleAHBotItemsRatioCommand(const char* args)
{
    char * opt = strtok((char*)args, " ");
    char * ahMapIdStr = strtok(NULL, " ");
    // uint32 qVal = (uint32) ;
    uint32 qVal = (uint32) strtoul(ahMapIdStr, NULL, 0);
    if (!qVal)
        return false;
...
template<int H>
bool ChatHandler::HandleAHBotItemsRatioHouseCommand(const char* args)
{
    char * opt = strtok((char*)args, " ");
    char * ahMapIdStr = strtok(NULL, " ");
    
    uint32 rVal;
    if((rVal = (uint32) strtoul(ahMapIdStr, NULL, 0)) == false)
        return false;
    // uint32 rVal;
...
template bool ChatHandler::HandleAHBotItemsRatioHouseCommand<AUCTION_HOUSE_ALLIANCE>(const char*);
template bool ChatHandler::HandleAHBotItemsRatioHouseCommand<AUCTION_HOUSE_HORDE>(const char*);
template bool ChatHandler::HandleAHBotItemsRatioHouseCommand<AUCTION_HOUSE_NEUTRAL>(const char*);

bool ChatHandler::HandleAHBotRebuildCommand(const char* args)
{
    bool all = false;
    if (*args)
    {
...
bool ChatHandler::HandleAHBotReloadCommand(const char* args)
{
...
bool ChatHandler::HandleAHBotStatusCommand(const char* args)
{
    bool all = false;
    if (*args)
    {
    ...
    PSendSysMessage(fmtId, GetSkyFireString(LANG_AHBOT_STATUS_ITEM_COUNT),
    ...
    if (all)
    {
        PSendSysMessage(fmtId, GetSkyFireString(LANG_AHBOT_STATUS_ITEM_RATIO),
        ...
        for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
            PSendSysMessage(fmtId, GetSkyFireString(ahbotQualityIds[i]),
